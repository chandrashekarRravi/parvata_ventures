...
export const socialLinks = [
  {
    label: "Instagram",
    href: "https://www.instagram.com/parvatha_global.ventures/",
    icon: InstagramIcon,
    color: "hover:text-[#E1306C] hover:border-[#E1306C]",
  },
  // {
  //   label: "LinkedIn",
  //   href: "https://www.linkedin.com/company/parvathaglobalventures",
  //   icon: LinkedInIcon,
  //   color: "hover:text-[#0A66C2] hover:border-[#0A66C2]",
  // },
  // {
  //   label: "Facebook",
  //   href: "https://www.facebook.com/parvathaglobalventures",
  //   icon: FacebookIcon,
  //   color: "hover:text-[#1877F2] hover:border-[#1877F2]",
  // },
  // {
  //   label: "YouTube",
  //   href: "https://www.youtube.com/@parvathaglobalventures",
  //   icon: YouTubeIcon,
...
// ── Reusable Social Row Component ─────────────────────────────────────────────

interface SocialRowProps {
  /** Size of each icon button. Defaults to "md" */
  size?: "sm" | "md" | "lg";
  className?: string;
}

export default function SocialRow({ size = "md", className = "" }: SocialRowProps) {
  const sizeClasses = {
    sm: "w-8 h-8",
    md: "w-10 h-10",
    lg: "w-12 h-12",
    ...
    <div className={`flex gap-3 flex-wrap ${className}`}>
      {socialLinks.map(({ label, href, icon: Icon, color }) => (
        <Link
          ...
          aria-label={label}
          className={`${sizeClasses[size]} flex items-center justify-center border border-outline/30 rounded-full text-on-surface-variant transition-all duration-300 ${color}`}
        >
